...
static void Usage() {
  ...
  printf(
      ...
      getvalidarglist('d')
          .c_str()); // TODO: Use this function also for other options ?
  printf(
      ...
      "        Enable/disable/change device setting: aam,[N|off], "
      "apm,[N|off],\n"
  ...
  printf(
      ...
      "        [default is +%s",
      get_valid_firmwarebug_args(), get_drivedb_path_add());
  ...
  printf(
      ...
      "=====\n\n"
      "  -t TEST, --test=TEST\n"
      "        Run test. TEST: offline, short, long, conveyance, force, "
      "vendor,N,\n"
...
static void scan_devices(const smart_devtype_list &types, bool with_open,
                         char **argv);
...
static const char *parse_options(int argc, char **argv,
  ...
  while ((optchar = getopt_long(argc, argv, shortopts, longopts, 0)) != -1) {
    ...
    case 'S':
      if (!strcmp(optarg, "on")) {
        ...
        ataopts.smart_auto_save_disable = scsiopts.smart_auto_save_disable =
            false;
      } else if (!strcmp(optarg, "off")) {
        ataopts.smart_auto_save_disable = scsiopts.smart_auto_save_disable =
            true;
        ataopts.smart_auto_save_enable = scsiopts.smart_auto_save_enable =
            false;
      } else {
        badarg = true;
      }
      break;
    case 'H':
      ataopts.smart_check_status = scsiopts.smart_check_status =
    ...
    case 'l':
      if (str_starts_with(optarg, "error")) {
        ...
        sscanf(optarg, "%*[a-z],0x%x%n,%u%n%c%u%n", &logaddr, &n1, &page, &n2,
               &sign, &nsectors, &n3);
        ...
        if (n == len && page <= 0xff && 0 < size && size <= 0x4000) {
          nvmeopts.log_page = page;
          nvmeopts.log_page_size = size;
        } else
          badarg = true;
      }
    ...
    case opt_identify:
      ...
      if (optarg) {
        for (int i = 0; optarg[i]; i++) {
          switch (optarg[i]) {
          case 'w':
            ataopts.identify_word_level = 1;
            break;
          case 'n':
            ataopts.identify_bit_level = -1;
            break;
          case 'v':
            ataopts.identify_bit_level = 1;
            break;
          case 'b':
            ataopts.identify_bit_level = 2;
            break;
          default:
            badarg = true;
          }
    ...
    case 'P':
      if (!strcmp(optarg, "use")) {
        ...
        if (optind < argc) { // -P showall MODEL [FIRMWARE]
          int cnt = showmatchingpresets(
              argv[optind], (optind + 1 < argc ? argv[optind + 1] : NULL));
      ...
      if (!strcmp(optarg, "offline")) {
        ...
        if (split_selective_arg(optarg, &start, &stop, &mode)) {
          ...
          if (ataopts.smart_selective_args.num_spans >= 5 || start > stop) {
            if (start > stop) {
              snprintf(extraerror, sizeof(extraerror),
                       "ERROR: Start LBA (%" PRIu64 ") > ending LBA (%" PRId64
                       ") in argument \"%s\"\n",
                       start, stop, optarg);
            } else {
              snprintf(
          ...
          ataopts.smart_selective_args
              .span[ataopts.smart_selective_args.num_spans]
              .start = start;
          ataopts.smart_selective_args
              .span[ataopts.smart_selective_args.num_spans]
              .end = stop;
    ...
    case 'f':
      if (!strcmp(optarg, "old")) {
        ...
        ataopts.output_format |=
            ata_print_options::FMT_HEX_ID | ata_print_options::FMT_HEX_VAL;
    ...
    case opt_set:     // --set
    {
      ...
      unsigned val;
      int n1 = -1, n2 = -1, n3 = -1, len = strlen(optarg);
      if (sscanf(optarg, "%16[^,=]%n%*[,=]%n%u%n", name, &n1, &n2, &val, &n3) >=
              1 &&
          (n1 == len || (!get && n2 > 0))) {
        bool on = false;
        bool off = false;
    ...
    if (badarg) {
      ...
      pout("=======> INVALID ARGUMENT TO -%s: %s\n",
           (optchar == opt_identify
                ? "-identify"
                : optchar == opt_set
                      ? "-set"
                      : optchar == opt_smart ? "-smart" : optstr),
           optarg);
  ...
  if (testcnt > 1) {
    printing_is_off = false;
    printslogan();
    pout("\nERROR: smartctl can only run a single test type (or abort) at a "
         "time.\n");
    UsageSummary();
    EXIT(FAILCMD);
  }
  ...
  if ((ataopts.smart_selective_args.pending_time ||
       ataopts.smart_selective_args.scan_after_select) &&
      !ataopts.smart_selective_args.num_spans) {
    printing_is_off = false;
    printslogan();
    if (ataopts.smart_selective_args.pending_time)
      ...
      pout("\nERROR: smartctl -t afterselect,(on|off) must be used with -t "
           "select,N-M.\n");
    UsageSummary();
    EXIT(FAILCMD);
  }
  ...
  if (argc - optind < 1) {
    pout("ERROR: smartctl requires a device name as the final command-line "
         "argument.\n\n");
    UsageSummary();
    EXIT(FAILCMD);
  }
  ...
  if (argc - optind > 1) {
    int i;
    pout("ERROR: smartctl takes ONE device name as the final command-line "
         "argument.\n");
...
void scan_devices(const smart_devtype_list &types, bool with_open,
                  char **argv) {
    ...
    if (with_open) {
      ...
      if (!dev->is_open()) {
        pout("# %s -d %s # %s, %s device open failed: %s\n",
             dev->get_dev_name(), dev->get_dev_type(), dev->get_info_name(),
...
static int main_worker(int argc, char **argv) {
  ...
  const char *type =
      parse_options(argc, argv, ataopts, scsiopts, nvmeopts, print_type_only);
  ...
  if (print_type_only)
    ...
    pout("%s: Device of type '%s' [%s] detected\n", dev->get_info_name(),
         dev->get_dev_type(), get_protocol_info(dev.get()));
...
int main(int argc, char **argv) {
  ...
  if (badcode)
    printf("Please inform " PACKAGE_BUGREPORT
