...
struct ata_log_request {
  ...
  ata_log_request() : gpl(false), logaddr(0), page(0), nsectors(0) {}
};
...
struct ata_print_options {
  ...
  int smart_selftest_type;   // OFFLINE_FULL_SCAN, ..., see atacmds.h. -1 for no
                             // test
  bool smart_selftest_force; // Ignore already running test
  ata_selective_selftest_args
      smart_selective_args; // Extra args for selective self-test
  ...
  int set_aam;       // disable(-1), enable(1..255->0..254) Automatic Acoustic
                     // Management
  ...
  int sct_wcache_sct_set;  // determined by ata set features command(1), force
                           // enable(2), force disable(3)
  bool sct_wcache_sct_set_pers; // persistent or volatile
  bool get_dsn;                 // print DSN status
  int set_dsn;                  // disable(02h), enable(01h) DSN

  ata_print_options()
      : drive_info(false), identify_word_level(-1), identify_bit_level(-1),
        smart_check_status(false), smart_general_values(false),
        smart_vendor_attrib(false), smart_error_log(false),
        smart_selftest_log(false), smart_selective_selftest_log(false),
        gp_logdir(false), smart_logdir(false), smart_ext_error_log(0),
        smart_ext_selftest_log(0), retry_error_log(false),
        retry_selftest_log(false), devstat_all_pages(false),
        devstat_ssd_page(false), pending_defects_info(false),
        pending_defects_log(0), sct_temp_sts(false), sct_temp_hist(false),
        sct_erc_get(false), sct_erc_set(false), sct_erc_readtime(0),
        sct_erc_writetime(0), sataphy(false), sataphy_reset(false),
        ...
        smart_selftest_type(-1), smart_selftest_force(false), sct_temp_int(0),
        sct_temp_int_pers(false), output_format(0), fix_swapped_id(false),
        ignore_presets(false), show_presets(false), powermode(0), powerexit(0),
        get_set_used(false), get_aam(false), set_aam(0), get_apm(false),
        set_apm(0), get_lookahead(false), set_lookahead(0), set_standby(0),
        set_standby_now(false), get_security(false), set_security_freeze(false),
        get_wcache(false), set_wcache(0), sct_wcache_reorder_get(false),
        sct_wcache_reorder_set(0), sct_wcache_reorder_set_pers(false),
        sct_wcache_sct_get(false), sct_wcache_sct_set(0),
        sct_wcache_sct_set_pers(false), get_dsn(false), set_dsn(0) {}
};
